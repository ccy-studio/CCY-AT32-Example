...
static sd_memory_card_type card_type =
...
sd_error_status_type sd_switch(uint32_t mode,
                               uint32_t group,
                               uint8_t value,
                               uint8_t* rsp);
...
sd_error_status_type sd_init(void) {
    ...
    if ((SDIO_MULTIMEDIA_CARD == card_type) &&
        (sd_card_info.sd_csd_reg.spec_version >= 4)) {
    ...
    if (status == SD_OK && ((SDIO_STD_CAPACITY_SD_CARD_V1_1 == card_type) ||
                            ...
                            (SDIO_SECURE_DIGITAL_IO_COMBO_CARD == card_type) ||
    ...
    printf(
        "card_capacity= %ld,card_blk_size = %ld,card_type = %d ;status=%d\r\n",
        sd_card_info.card_capacity, sd_card_info.card_blk_size,
        sd_card_info.card_type, (status == SD_OK));
    return status;
}
 ...
 * @param  card_info: pointer to a sd_card_info_struct_type structure that
...
sd_error_status_type sd_card_info_get(sd_card_info_struct_type* card_info) {
    ...
    if ((card_type == SDIO_STD_CAPACITY_SD_CARD_V1_1) ||
        ...
        card_info->card_capacity *=
            (1 << (card_info->sd_csd_reg.device_size_mult + 2));
        card_info->card_blk_size =
            1 << (card_info->sd_csd_reg.max_read_blk_length);
        ...
        card_info->card_capacity =
            (uint64_t)(card_info->sd_csd_reg.device_size + 1) * 512 * 1024;
 ...
 * @retval sd_error_status_type: sd card error code.
 */
sd_error_status_type sdio_command_data_send(
    sdio_command_struct_type* sdio_cmd_init_t,
 ...
 * @retval sd_error_status_type: sd card error code.
 */
sd_error_status_type sd_block_read(uint8_t* buf,
                                   long long addr,
                                   uint16_t blk_size) {
    sd_error_status_type status = SD_OK;
    uint32_t response = 0;
 ...
 * @retval sd_error_status_type: sd card error code.
 */
sd_error_status_type sd_mult_blocks_read(uint8_t* buf,
                                         long long addr,
                                         uint16_t blk_size,
                                         uint32_t nblks) {
    sd_error_status_type status = SD_OK;
 ...
 * @retval sd_error_status_type: sd card error code.
 */
sd_error_status_type sd_block_write(const uint8_t* buf,
                                    long long addr,
                                    uint16_t blk_size) {
    sd_error_status_type status = SD_OK;
 ...
 * @retval sd_error_status_type: sd card error code.
 */
sd_error_status_type sd_mult_blocks_write(const uint8_t* buf,
                                          long long addr,
                                          uint16_t blk_size,
                                          uint32_t nblks) {
    sd_error_status_type status = SD_OK;
 ...
 * @retval sd_error_status_type: sd card error code.
 */
sd_error_status_type mmc_stream_read(uint8_t* buf,
                                     long long addr,
                                     uint32_t len) {
    uint32_t response = 0;
 ...
 * @retval sd_error_status_type: sd card error code.
 */
sd_error_status_type mmc_stream_write(uint8_t* buf,
                                      long long addr,
                                      uint32_t len) {
    sd_error_status_type status = SD_OK;
    uint32_t response = 0;
...
sd_error_status_type sd_irq_service(void) {
    ...
    sdio_interrupt_enable(
        SDIOx,
        (SDIO_DTFAIL_INT | SDIO_DTTIMEOUT_INT | SDIO_DTCMP_INT |
         SDIO_TXBUFH_INT | SDIO_RXBUFH_INT | SDIO_TXERRU_INT | SDIO_RXERRO_INT |
         SDIO_SBITERR_INT),
        FALSE);
    return (SD_OK);
}
...
sd_error_status_type command_rsp1_error(uint8_t cmd) {
    ...
    return (sd_error_status_type)(sdio_response_get(SDIOx, SDIO_RSP1_INDEX) &
                                  SD_OCR_ERRORBITS);
}
...
sd_error_status_type check_card_programming(uint8_t* p_status) {
    ...
    while (!(sts_reg & (SDIO_CMDFAIL_FLAG | SDIO_CMDTIMEOUT_FLAG |
...
sd_error_status_type speed_change(uint8_t speed) {
    ...
    /* check card type */
    if ((SDIO_STD_CAPACITY_SD_CARD_V1_1 == card_type) ||
        (SDIO_STD_CAPACITY_SD_CARD_V2_0 == card_type) ||
        (SDIO_HIGH_CAPACITY_SD_CARD == card_type)) {
